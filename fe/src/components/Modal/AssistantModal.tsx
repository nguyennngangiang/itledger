...
// Grounded Q&A over the fleet, answered by the local LLM (RAG). Kept mounted so
    ...
    try {
      ...
      setMsgs((m) => [
        ...m,
        {
          role: "ai",
          text:
            "⚠️ The local LLM is unavailable. Make sure the model is pulled and the AI service is running.",
        },
      ]);
    } finally {
      setLoading(false);
          ...
          {msgs.length === 0 && (
            <div className="assistant-empty">
              ...
              <p>
                ...
                live inventory by the local LLM (qwen2.5) — nothing leaves this
                machine.
              </p>
          ...
          {loading && (
            <div className="assistant-msg ai">
              <div className="assistant-bubble assistant-thinking">
                thinking… (local LLM — can take a moment on CPU)
              </div>
            </div>
          )}
        </div>
